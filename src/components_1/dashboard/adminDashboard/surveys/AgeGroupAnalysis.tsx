...
export default function AgeGroupAnalysis({
  ...
  const hasData = ageChartData && ageChartData.length > 0;
  ...
  const participantsDistribution = ageChartData.map((age, index) => ({
    ...
    percentage: Math.round((age.participants / totalParticipants) * 100),
  }));
  ...
  if (!hasData) {
    return (
      <EmptyState
        ...
        message="No age group data available with sufficient participant count to maintain privacy."
      />
    );
  }
